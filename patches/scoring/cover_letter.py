...
# Max cross-run retries before giving up. Both cover and tailor use this.
# If a job's tailor_attempts/cover_attempts reaches this cap, it's skipped
# on subsequent runs (likely a permanent failure — bad URL, persistent
# LLM error, etc.) until you manually reset the counter.
MAX_ATTEMPTS = 5
# Backwards-compatible alias (an older revision of this file referenced
# MAX_COVER_ATTEMPTS directly).
MAX_COVER_ATTEMPTS = MAX_ATTEMPTS
...
def run_cover_letters(min_score: int = 7, limit: int = 20,
    ...
    Args:
        ...
        validation_mode: "strict", "normal", or "lenient".
        since:           ISO datetime; only jobs discovered at/after this are
                         considered (used by daily cron for a 24h window).
    ...
    params: list = [min_score, MAX_COVER_ATTEMPTS]
    ...
    for job in jobs:
        ...
        try:
            ...
            # Best-effort: also build a combined Application Packet PDF
            # (resume + cover in one file). Skips silently if the resume
            # PDF doesn't exist (e.g. when this stage is run alone).
            try:
                from applypilot.scoring.pdf import combine_packet_pdf
                # TAILORED_DIR uses site_title naming, cover uses site_title_CL.
                # The two stems differ so we need to find the matching resume.
                # The simplest approach: the cover_letter stage already runs
                # AFTER tailor, so the resume PDF exists with stem prefix.
                resume_pdf = TAILORED_DIR / f"{prefix}.pdf"
                if resume_pdf.exists() and pdf_path:
                    combine_packet_pdf(
                        resume_pdf, pdf_path,
                        TAILORED_DIR / f"{prefix}_PACKET.pdf",
                    )
            except Exception:
                log.debug("Packet PDF generation failed for %s", cl_path, exc_info=True)
