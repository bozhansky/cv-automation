...
def _resolve_app_dir() -> Path:
    """Pick the real APP_DIR.

    Hermes profile isolation sets $HOME to a sandboxed path that doesn't
    contain the user's actual files. We try several candidate APP_DIRs in
    order and pick the first that contains applypilot.db. (The real one is
    at /home/bostjan/.applypilot which is a symlink to the CV-automation
    project.) Env var APPLYPILOT_DIR wins over auto-detection.
    """
    env = os.environ.get("APPLYPILOT_DIR", "").strip()
    candidates = []
    if env:
    ...
    for c in candidates:
        if (c / "applypilot.db").exists():
            return c
    # None of them have the DB — fall back to the first candidate and let
    # the caller create it (matches the original behaviour).
    return candidates[0]
...
def get_tier() -> int:
    ...
    Tier 3 (Full Auto-Apply):       + Ollama + Chrome
    """
    load_env()
    ...
    has_legacy_cli = shutil.which("legacy-cli") is not None
    try:
        get_chrome_path()
        has_chrome = True
    ...
    if has_legacy_cli and has_chrome:
        return 3
...
def check_tier(required: int, feature: str) -> None:
    ...
    if required >= 3:
        if not shutil.which("legacy-cli"):
            missing.append("Ollama (already running) + Chrome — make sure Ollama is reachable on http://127.0.0.1:11434 and Chrome/Chromium is installed")
        try:
            get_chrome_path()
        except FileNotFoundError:
