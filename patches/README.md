...
| `apply/notifier.py` | NEW — Telegram notifications | `notify_applied()`, `notify_failed()`, `notify_approval_needed()` (with inline-keyboard buttons), `answer_callback_query()`, threaded send, env-var-driven config (supports both `TELEGRAM_*` and `APPLY_TELEGRAM_*` env var names); SHA-256 URL-hash registry for long-URL button callbacks |
| `scripts/telegram_callback_daemon.py` | NEW — Telegram callback polling daemon (4.6) | Long-polls `getUpdates` for `callback_query` events from `notify_approval_needed()`; calls `mark_approval_approved` / `mark_approval_declined`; manages its own PID + log files; signal-safe shutdown (SIGTERM/SIGINT) |
