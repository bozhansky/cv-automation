...
| `database.py` | Schema migrations + URL dedup + per-site analytics + dynamic blacklist + parameterized max_attempts | `ensure_columns()` for `approved_at`/`apply_cost_usd`; `ensure_indexes()`; `_canonicalize_url()`; `purge_old_jobs()`; `get_site_stats()`; `get_dynamic_blacklist()`; `is_site_blacklisted()`; `get_blacklist_as_dict()`; 3-layer dedup in `store_jobs()` (canonicalize + in-batch + DB PRIMARY KEY); `get_jobs_by_stage(max_attempts=5)` parameterizes the per-job retry cap so it's consistent across `pending_tailor` and `cover_letter.py` |
| `scoring/cover_letter.py` | MAX_COVER_ATTEMPTS NameError fix | The SQL on line 215 referenced `MAX_COVER_ATTEMPTS` but the module-level constant was `MAX_ATTEMPTS`. Added `MAX_COVER_ATTEMPTS = MAX_ATTEMPTS` alias so the existing SQL resolves. Both `tailor.py` and `cover_letter.py` use the same `MAX_ATTEMPTS = 5` cap. |
