...
Constructs the full instruction prompt that tells Legacy agent / the AI agent
...
def build_prompt(job: dict, tailored_resume: str,
    ...
    if dry_run:
        submit_instruction = (
            "IMPORTANT: Do NOT call submit_application — it is disabled in dry-run. "
            "Do not click any final Submit/Apply button either. Instead, review the form, "
            "verify all fields are correct, then call finish(FAILED, dry-run) with a short note."
        )
    else:
        submit_instruction = (
            "BEFORE you submit, you MUST follow this exact sequence:\n"
            "  (a) Fill all required fields.\n"
            "  (b) Call browser_take_screenshot and visually verify every field matches the APPLICANT PROFILE and TAILORED RESUME.\n"
            "  (c) If anything is wrong or missing, fix it FIRST and re-screenshot.\n"
            "  (d) Call submit_application(screenshot_taken=True, fields_verified='list,of,fields', reason='one-line summary') as the very NEXT tool call.\n"
            "  (e) After receiving 'SUBMIT APPROVED', click the Submit/Apply button via browser_click, then call finish(APPLIED).\n"
            "WARNING: The submit_application tool is a safety gate. It will REFUSE if you call any other tool between the screenshot and the submit. If you need to act after the screenshot (e.g. fix a field), take a fresh screenshot first."
        )

    # Cached form schema (4.8) — inject cached selectors for this site
    # so the agent doesn't waste turns rediscovering the form.
    cached_schema_section = ""
    try:
        from applypilot.apply.form_schema_cache import get_schema_for_prompt
        site = job.get("site") or ""
        if site:
            cached_schema_section = get_schema_for_prompt(site)
    except Exception as e:
        # Non-fatal — proceed without cache
        logger = __import__("logging").getLogger(__name__)
        logger.debug("build_prompt: failed to load form schema cache: %s", e)
