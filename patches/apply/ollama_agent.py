"""Ollama-based apply agent — replaces Legacy CLI subprocess.
...
the Legacy CLI subprocess if APPLY_AGENT=legacy-cli.

Event stream (matches the stream-json format) is emitted on stdout
...
# conservative token-cost estimate ($3/1M input, $15/1M output — standard
...
def run_ollama_agent(
    ...
    Args:
        ...
        emit_event: Optional callable(str) that receives stream-json lines
                    (matches Legacy CLI format) for the dashboard
    ...
    # Emit final result event (matches Legacy CLI format)
    est_cost = (
        stats["input_tokens"] * COST_PER_INPUT_TOKEN
        + stats["output_tokens"] * COST_PER_OUTPUT_TOKEN
    )
