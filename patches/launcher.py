...
def run_job(job: dict, port: int, worker_id: int = 0,
            model: str = "sonnet", dry_run: bool = False) -> tuple[str, int, float]:
    """Spawn an apply agent for one job application.

    Routes to the Ollama-based Python agent loop when APPLY_AGENT=ollama
    (default in this fork), or falls back to spawning the Claude CLI
    subprocess when APPLY_AGENT=claude.
    ...
    if apply_agent == "claude":
        return _run_job_claude_cli(job, port, worker_id, model, agent_prompt)
    ...
    # Normalize status string (matches what Claude CLI returned)
    status_map = {
        "APPLIED": "applied",
        "EXPIRED": "expired",
        "CAPTCHA": "captcha",
...
def _run_job_claude_cli(job: dict, port: int, worker_id: int,
                        model: str, agent_prompt: str) -> tuple[str, int, float]:
    """Original Claude-CLI-based apply path (kept for fallback)."""
    # Write per-worker MCP config
    mcp_config_path = config.APP_DIR / f".mcp-apply-{worker_id}.json"
    mcp_config_path.write_text(json.dumps(_make_mcp_config(port)), encoding="utf-8")
    # Cost is metered by Anthropic; we report 0.0 for the Claude CLI path.
    # (Future: parse the Claude CLI usage output for token counts.)
